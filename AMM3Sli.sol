...
contract CPAMM {
    ...
    mapping(address => mapping(address => uint)) public reserve0;//第一个address是lptoken的address ，第2个是相应token的资产，uint是资产的amount
    mapping(address => mapping(address => uint)) public reserve1;

    //检索lptoken
    mapping(address => mapping(address => address)) public findLpToken;

    function setSli(uint dx, uint x, uint y, uint _disirSli) public pure returns(uint){
    ...
    function swapByLimitSli(address _tokenIn, address _tokenOut, uint _amountIn, uint _disirSli) public {
        ...
        uint reserveIn = reserve0[lptokenAddr][_tokenIn];
        uint reserveOut = reserve1[lptokenAddr][_tokenOut];
        ...
        uint totalReserve0 = reserve0[lptokenAddr][_tokenIn] + _amountIn; 
        uint totalReserve1 = reserve1[lptokenAddr][_tokenOut] - amountOut;
    ...
    function _update(address lptokenAddr,address _token0, address _token1, uint _reserve0, uint _reserve1) private {
        reserve0[lptokenAddr][_token0] = _reserve0;
        reserve1[lptokenAddr][_token1] = _reserve1;
    }
    ...
    function addLiquidity(address _token0, address _token1, uint _amount0, uint _amount1) public returns (uint shares) {
        ...
        */
        if (findLpToken[_token1][_token0] == address(0)) {
            ...
            lptoken = ILPToken(lptokenAddr);//获取lptoken地址
            shares = _min(
                (_amount0 * lptoken.totalSupply()) / reserve0[lptokenAddr][_token0],
                (_amount1 * lptoken.totalSupply()) / reserve1[lptokenAddr][_token1]
            );
            //获取lptoken地址
        }
        ...
        _update(lptokenAddr,_token0, _token1, reserve0[lptokenAddr][_token0] + _amount0, reserve1[lptokenAddr][_token1] + _amount1);
    }
    /*
        撤回流动性
    撤回的token0和token1 =
    ...
    */
    function removeLiquidity(
        ...
        */
        ...
        amount0 = (_shares * reserve0[lptokenAddr][_token0]) / lptoken.totalSupply();//share * totalsuply/bal0
        amount1 = (_shares * reserve1[lptokenAddr][_token1]) / lptoken.totalSupply();
        require(amount0 > 0 && amount1 > 0, "amount0 or amount1 = 0");

        lptoken.burn(msg.sender, _shares);
        _update(lptokenAddr,_token0, _token1, reserve0[lptokenAddr][_token0] - amount0, reserve1[lptokenAddr][_token1] - amount1);
